...
const RemittanceHistory = () => {
  ...
  const fetchRemittances = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/remittances');
  ...
  const handleExportPDF = () => {
    ...
    exportTableToPDF('Remittance History', headers, remittances, mapper, `Remittance_History_${new Date().toISOString().split('T')[0]}`);
  };

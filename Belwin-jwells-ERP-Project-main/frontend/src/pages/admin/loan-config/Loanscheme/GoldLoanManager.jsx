...
const GoldLoanManager = ({ showAddForm, setShowAddForm }) => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setFormData({
          ...
          interestRepaymentMonths: '', documentCharges: '', penalty: '', 
          schemeType: 'Bellwin Gold Loan', status: 'Active'
        });
  ...
  const generateNextSchemeId = () => {
    if (!schemes || schemes.length === 0) return 'GL-001';
    let maxNum = 0;
    schemes.forEach(s => {
      if (s.schemeId && s.schemeId.startsWith('GL-')) {
        const num = parseInt(s.schemeId.split('-')[1]);
        if (!isNaN(num) && num > maxNum) maxNum = num;
      }
    });
    return `GL-${String(maxNum + 1).padStart(3, '0')}`;
  };
    ...
    <>
      {showAddForm && (
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6 animate-fade-in">
          <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">New Bellwin Gold Loan Details</h3>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead className="bg-gray-50 border-y border-gray-200 text-gray-600">
              <tr>
                ...
                <th className="p-3 text-xs font-bold uppercase tracking-wider">Gram Rate</th>
                <th className="p-3 text-xs font-bold uppercase tracking-wider">Mimim Gram</th>
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {schemes.length === 0 && (
                <tr>
                  <td colSpan="9" className="p-8 text-center text-gray-500">No Bellwin Gold Loans found. Use the form above to add one.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default GoldLoanManager;

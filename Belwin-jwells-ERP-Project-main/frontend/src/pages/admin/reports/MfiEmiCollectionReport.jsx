...
const MfiEmiCollectionReport = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter MFI collections based on some indicator. 
      // If backend doesn't provide loanType in collection response, we might need a dummy filter.
      // For now, we will just use the data and assume MFI loans are identifiable by ID prefix or type if provided.
      ...
      console.error(error);
      toast.error('Failed to fetch MFI EMI collection report');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 animate-fade-in">
      <PageHeader 
        title="MFI Loan EMI Collection Report" 
        subtitle="Track branch-wise and employee-wise daily/monthly MFI collections." 
        icon={FileText} 
        actions={
          <div className="flex gap-2">
            ...
            <Button variant="primary" icon={Download} onClick={() => {
              ...
              exportTableToPDF('MFI EMI Collection Report', headers, data, 'MFI_EMI_Collections');
            }}>Export PDF</Button>
          </div>
        }
      />
...
export default MfiEmiCollectionReport;
